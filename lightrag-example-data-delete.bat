...
REM 重置Milvus
echo 正在重置Milvus...
python "%PY_SCRIPT%" "reset_milvus"
if errorlevel 1 (
    echo [ERROR] 重置Milvus失败，请检查日志
    exit /b 1
)
...
REM 删除Neo4j数据
echo 正在删除Neo4j数据...
python "%PY_SCRIPT%" "delete_neo4j_data"
if errorlevel 1 (
...
REM 清理MongoDB
echo 正在清理MongoDB...
python "%PY_SCRIPT%" "drop_mongo_db"
if errorlevel 1 (
    echo [ERROR] 清理MongoDB失败，请检查MongoDB连接
    exit /b 1
)
